...
exports.POST = (req, res) => {
  ...
  const sql = `
    ...
    ;
  `;
  db.query(sql, [req.user.userid, paymentId], (err, result) => {
    if (err) {
      console.log(err);
      return res
        .status(500)
        .send({ msg: "unable to query for payment amount", msgType: "error" });
    }

    if (!result.length)
      return res
        .status(404)
        .send({ msg: "no record of paypal payment", msgType: "error" });
                          ...
                          (err, result) => {
                            if (err) {
                              ...
                              return res
                                ...
                                .send({
                                  msg:
                                    "unable to determine whether coupon was used",
                                  msgType: "error",
                                });
                            }
                                ...
                                (err, result) => {
                                  if (err) {
                                    ...
                                    return res
                                      ...
                                      .send({
                                        msg:
                                          "unable to designate coupon as used",
                                        msgType: "error",
                                      });
                                  }
