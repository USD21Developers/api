...
exports.POST = (req, res) => {
  ...
  db.query(sql, [req.user.userid], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .send({ msg: "unable to query for user country", msgType: "error" });
    }

    if (!result.length)
      return res.status(404).send({ msg: "user not found", msgType: "error" });

    const country = result[0].country.toUpperCase() || "US";
...
`;
    db.query(sql, [req.user.userid, couponCode, couponCode], (err, result) => {
      ...
      // https://github.com/paypal/PayPal-node-SDK/blob/965a132d7f5d5880840e55d82f9dbc80eb44e159/samples/payment_experience/web_profile/create_payment_with_customized_experience.js
      let localeCode = country === "US" ? "en_US" : `${lang}_XC`;
